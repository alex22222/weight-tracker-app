#!/bin/bash
# 准备 CloudBase 部署包 - 修正版（包含 Dockerfile）
...
2. Dockerfile 路径: ./Dockerfile
3. 构建命令: 使用 Dockerfile 自动构建
4. 端口: 3000
...
ls -lh deploy.zip
echo ""
echo "✅ 包含 Dockerfile，可以上传到 CloudBase 了！"
